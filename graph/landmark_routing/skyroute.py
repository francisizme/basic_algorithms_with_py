...
landmark_string = '\n'.join(f"{choice} - {name}" for choice, name in landmark_choices.items())
stations_under_construction = ["Stadium-Chinatown", "Broadway-City Hall", "Commercial-Broadway"]
...
def new_route(start_point=None, end_point=None):
    ...
    if shortest_route is not None:
        print("The shortest metro route from {0} to {1} is:\n{2}".format(start_point, end_point, shortest_route))
    else:
...
def get_route(start_point, end_point):
    start_stations, end_stations = vc_landmarks[start_point], vc_landmarks[end_point]
    routes = []
    ...
    return ' -> '.join(shortest_route)


def get_active_stations():
    updated_metro = vc_metro
    for station_under_construction in stations_under_construction:
        for current_station, neighbor_stations in vc_metro.items():
